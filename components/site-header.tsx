...
export function SiteHeader() {
  ...
  </div><nav id="mobile-navigation" aria-label="Mobile navigation" className={cn('mobile-nav lg:hidden', open && 'is-open')}><div className="site-container flex flex-col py-8">{navItems.map((item) => <Link key={item.href} href={item.href} onClick={() => setOpen(false)}>{item.label}</Link>)}<Link href="/contact" className="text-[var(--signal)]" onClick={() => setOpen(false)}>Discuss your event</Link></div></nav></header>;
}
