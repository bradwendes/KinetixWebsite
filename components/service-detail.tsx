...
type Experience = { stat:string; title:string; text:string };
type Props = { eyebrow:string; title:string; intro:string; heading:string; paragraphs:string[]; capabilities:string[]; image?:boolean; experience?:Experience[]; experienceTitle?:string; process:{title:string;text:string}[]; cta:string };
export function ServiceDetail({ eyebrow,title,intro,heading,paragraphs,capabilities,image=false,experience,experienceTitle='Selected experience',process,cta }:Props) { return <>
  ...
  {experience && <section className="section-pad bg-[var(--purple)]"><div className="site-container"><p className="eyebrow">Experience</p><h2 className="display-heading max-w-4xl">{experienceTitle}</h2><div className="experience-grid mt-12">{experience.map((item)=><article className="experience-card" key={item.title}><strong>{item.stat}</strong><h3>{item.title}</h3><p>{item.text}</p></article>)}</div></div></section>}
